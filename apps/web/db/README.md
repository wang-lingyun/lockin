...
| `migrations/0007_mistakes_reflections.sql` | Stage 6 (PRD §10.9/§10.11): `mistake_bank_entries` (mistake to revisit; optional subject/track + optional `homework_submission_id` FK with `on delete set null`; status needs_review/reviewed/mastered; RLS, `updated_at` trigger) and `reflections` (four daily prompts + `parent_comment`, Pacific `date` default, `unique(student_id, date)`, RLS, `updated_at` trigger). No streak schema: the streak is computed on read (`lib/streak/computeStreak.ts`) from reflections + completed missions. |
| `migrations/0008_coding_rewards.sql` | Stage 7 (PRD §10.10/§10.12): `coding_projects` (status active/completed/archived, goal + demo/github links + reflection notes) and `coding_features` (checklist items; status not_started/in_progress/completed; `xp_awarded` idempotency guard; RLS via the parent project) and `rewards` (optional `required_xp`; reserved `unlocked`/`unlocked_at`). All RLS + `updated_at` triggers. RPC `set_coding_feature_status` awards 20 XP once on first completion (xp_events source_type `coding_feature`, bumps current_xp/level); RPC `adjust_student_xp` applies a manual parent delta (source_type `manual`, clamps current_xp ≥ 0). Reward unlock is derived on read (`current_xp ≥ required_xp`), not stored. |
