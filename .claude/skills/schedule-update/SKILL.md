...
bash .claude/skills/schedule-update/connect.sh -c "select id, name, grade from students order by created_at;"

# A multi-statement transaction from a heredoc:
bash .claude/skills/schedule-update/connect.sh <<'SQL'
begin;
-- … statements …
commit;
SQL
